...
import {
  ...
  Building2, Tag, Clock, X, Copy, Mail, ChevronDown, ChevronUp, Send, AtSign, AlertTriangle,
} from "lucide-react";
...
const DetailModal = ({ result, onClose, onMarkSent }: DetailModalProps) => {
  ...
  const handleSendReply = () => {
    ...
    if (!isSent && onMarkSent) {
      ...
      toast.success(`Reply sent — CC’d to ${ccSummary}`);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-5 space-y-4">
          {/* Escalation banner in modal */}
          {isEscalated && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 bg-[#E67E2210] border border-[#E67E2240] rounded-xl">
              <AlertTriangle className="w-3.5 h-3.5 text-[#E67E22] flex-shrink-0 mt-0.5" />
              <div>
                <div className="text-[11px] font-bold text-[#E67E22]">Escalated — Dean &amp; VC will be CC’d</div>
                <div className="text-[10px] text-[hsl(var(--muted-foreground))] mt-0.5">{result.escalationReason}</div>
              </div>
            </div>
          )}
            ...
            {typingDone && (
              <div className="border-t border-[hsl(var(--border))] px-4 py-2 bg-[hsl(var(--secondary)/0.3)]">
                ...
                >
                  ...
                  <span>Sending will CC <strong className="text-[hsl(var(--foreground))]">{ccSummary}</strong></span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-3 text-center">
                          ...
                          {result.escalated && (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-[#E67E2218] text-[#E67E22] border border-[#E67E2240]">
